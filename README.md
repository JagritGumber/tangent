...
- Rust workspace at [`rust/`](./rust/) with the [`tangent-sdk`](./rust/tangent-sdk/) crate shipping canonical EIP-712 orders, deployment-manifest parsing, signed `submitOrder` calldata, primitive contract calldata helpers, and single-word ABI return decoders. It is intentionally still a raw SDK surface, not a full RPC client. Future crates (`tangent-keeper`, `tangent-indexer`, `tangent-matcher`) are reserved in the workspace manifest, landing at v0.8 / v0.9 / v0.10 respectively.
