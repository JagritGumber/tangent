...
- Rust workspace at [`rust/`](./rust/) with the [`tangent-sdk`](./rust/tangent-sdk/) crate shipping canonical EIP-712 orders, deployment-manifest parsing, signed `submitOrder` calldata, primitive contract calldata helpers, typed account/collateral/market/order/settlement/liquidation workflow plans, fixed-order read decoders, decoded-status helpers, and `UnsignedCall` inspection helpers. It is intentionally still a raw, transport-agnostic SDK surface, not a full RPC client or signing backend. Future crates (`tangent-keeper`, `tangent-indexer`, `tangent-matcher`) are reserved in the workspace manifest, landing at v0.8 / v0.9 / v0.10 respectively.
